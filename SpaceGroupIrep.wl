...
showSGIrepDirectProduct::usage="showSGIrepDirectProduct[sgno, kcoord1, kcoord2]  shows the decomposition of the "<>
   "direct product of the SG ireps of kcoord1 star and SG ireps of kcoord2 star. Default options are "<>
...
getSGIrepTab[sgno_Integer, kNameOrCoord_, OptionsPattern[]]:=Block[{u,t\:2081,t\:2082,t\:2083,LGtabs,forOneK},
...
SGIrepDirectProduct[sgno_Integer, kcoord1_, kcoord2_, OptionsPattern[]]:=Block[{u,Ireps1,Ireps2,getchi,
  directProduct,brav,G,G0,times,inv,dtimes,dinv,rightCoset,doubleCosetRep,fixk3BZs},
  brav=getSGLatt[sgno];
  If[!VectorQ[kcoord1,NumericQ]||!VectorQ[kcoord1,NumericQ],
    Print["SGIrepDirectProduct: The input parameters should be the k-point coordinates not names."];
    Abort[];
  ];
  ...
  directProduct[reptab1_,reptab2_]:=Module[{kstar1,kstar2,k1k2,ktest,startest,i,j,tmp,kinfo1,kinfo2,
    ...
    DPs,strki,dims1,dims2,dims3,k3dict,k3BZs0},
    ...
    k3reps0=getLGCharTab[sgno,#,"abcOrBasVec"->OptionValue["abcOrBasVec"]]&/@k3s;
    k3BZs=<||>;   k3BZs0=<||>;
    For[i=1,i<=nk3,i++,
    ...
    If[Or@@(keqmod[k1,#]&/@kstar2), ir1ir2=Select[ir1ir2,#[[1]]<=#[[2]]&]];
    ...
    Do[
      ...
      For[i=1,i<=nk3,i++, k3=k3s[[i]];
        ...
        For[j=1,j<=Length[ir1ir2],j++, {ir1,ir2}=ir1ir2[[j]];
          ...
          DP[{ir1,ir2},"irep",strki]={tmp1,tmp2,tmp};
        ];
...
showSGIrepDirectProduct[sgno_Integer, kcoord1_, kcoord2_, OptionsPattern[]]:=Module[
  {DPs,forOneBZ,showk,bold,lbtype},
  DPs=SGIrepDirectProduct[sgno,kcoord1,kcoord2,"abcOrBasVec"->OptionValue["abcOrBasVec"]];
...
FullForm]\);
    ...
    show2[key_]:=Module[{rep,kis,reps,g},
      ...
      reps=Table[Row[{#1,Style[Row[{"(",ki,",",#2,")"}],Gray]}]&@@@Transpose[rep[ki][[{lbtype,3}]]], {ki,kis}];
      reps=Flatten[reps];
      (*
...
autoConvTraceToBC[poscarFile_,traceData_]:=autoConvTraceToBC[poscarFile,traceData,1*^-5]
autoConvTraceToBC[poscarFile_,traceData_,prec_]:=
 Module[{cell,BCcell,sym,ops1,ops2,ops3,brav,ckbv,trdat,sgno,U,Q,t0,S,R},
  ...
  If[ckbv[[1]]&&seteq[ops1,ops3], 
     Return[<|"trace"->traceData,"input_cell"->cell,"BCcell"->cell,"sym"->sym|>]
    ];
  {t0,U}=SGGenElem[sgno][[{2,3}]];
  {Q,S}=getQandS[sgno];
  ...
  <|"trace"->trdat,"input_cell"->cell,"BCcell"->BCcell,"sym"->sym|>
]
